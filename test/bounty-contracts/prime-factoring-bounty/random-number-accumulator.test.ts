...
describe('RandomNumberAccumulator', () => {
  ...
  const MAX_GAS_LIMIT = BigNumber.from('0xffffffffffffffff')
  ...
  // const _384BitPrime = BigNumber.from(arrayify('0xe63c9c85d3ef302557ec69dd7bb313cd03c39fa2ac6c261120a858d1ded315036be457ff7ebb7624038fb3eb6bed5c7d'))
  ...
  it.only('should append sequential numbers to reach the required bytes', async () => {
    ...
    expect(await randomNumberAccumulator.isDone()).to.be.eq(false)
    await randomNumberAccumulator.accumulate(secondHalf, { gasLimit: MAX_GAS_LIMIT })
    expect(await randomNumberAccumulator.isDone()).to.be.eq(true)
  })
