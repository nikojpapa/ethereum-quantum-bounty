...
contract SignatureBounty {
    ...
    constructor(address[] memory publicKeys) {
        locks = publicKeys;
    }
    ...
    function widthdraw(bytes32 message, bytes[] memory signatures) public requireUnsolved {
        _assertSignaturesMatchLocks(message, signatures);
        solved = true;
        _sendBountyToSolver();
    }

    function _assertSignaturesMatchLocks(bytes32 message, bytes[] memory signatures) private view {
